...
public class RippleTransHandler implements DivisibleTransHandler {
	...
	static class RippleTable {
		...
		private static final float rippleWaveForm(float rad) {
			...
			float s = ((FloatMath.sin(rad) + FloatMath.sin(rad*2-2) * 0.2f) / 1.19f);
			s *= s;
			return s;
		}
			...
			try {
				...
				for( int y = 0; y < mMapHeight; y++ ) {
					...
					for( int x = 0; x < mMapWidth; x++ ) {
						...
						int dir = (int) (Math.atan(xx*fac) * ((1.0f/(PI/2.0f)) * RIPPLE_DIR_PREC)); // dir = 方向コード
						int dist = (int)FloatMath.sqrt(xx*xx + yy*yy) & ripplemask; // dist = 中心からの距離
				...
				for( int w = 0; w < RIPPLE_DIR_PREC; w++ ) {
					float fdir =  (PI*0.5f - (((float)w + 0.5f) * ((1.0f / (float)RIPPLE_DIR_PREC) * (PI / 2.0f))));
					float v;
					v = (float) (FloatMath.cos(fdir) * 2048.0f);
					cos_table[w] = (int)(v < 0 ? v - 0.5f : v + 0.5f); // 1.11
					v = (float) (FloatMath.sin(fdir) * 2048.0f);
	...
	public int startProcess(long tick) {
		...
		mPhase = (int)(mSpeed * ((1.0f/(PI*2))*(1.0f/1000.0f)) * mCurTime * mRippleWidth) % mRippleWidth;
		if(mPhase < 0) mPhase = 0;
		mPhase = mRippleWidth - mPhase - 1;

		// Drift
		float s = (float) FloatMath.sin( PI * mCurTime / mTime);
