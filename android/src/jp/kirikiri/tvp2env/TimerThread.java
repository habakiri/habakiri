...
public class TimerThread implements Runnable {
	private final class TimerRunnable implements Runnable {
		...
		public void run() {
			if(!getTerminated() ) {
				// pending イベント発行
				synchronized( mTimerHander ) {	// thread-protected
	...
	private final EventHandleThread mHandler;
	TimerRunnable mTimerHander;
	...
	private TimerThread() {
		...
		mHandler = TVP.Application.get().getVMThread();
		mTimerHander = new TimerRunnable();
	...
	boolean getTerminated() { return mIsTerminated; }
