...
public class PNGLoader {
	...
	public static IndexedBitmap loadPNG( BinaryStream src, int mode ) throws TJSException {
		...
		L: while( src.getPosition() <= filesize ) {
			...
			switch( chunk_header ) {
			//case PLTE_CHUNK: // パレットの実体は読む必要がない
			//	palette = readPalette(src,size);
			//	break;
			case IDAT_CHUNK:
				if( image != null ) Message.throwExceptionMessage( "Not supported multi IDAT chunk." );
				image = readImage(src,size,width,height,bitdepth);
		...
		if( image == null ) Message.throwExceptionMessage( "Internal error." );

		if( bitdepth == 8) {
			IndexedBitmap result = new IndexedBitmap(width,height,image);
			return result;
		}
		// 8bit 以外の時は、拡張して返す
		...
		if( bitdepth == 4 ) {
			for( int y = 0; y < height; y++ ) {
				for( int x = 0; x < stride; x++ ) {
					...
					imagedata[o] = (byte)(idx >>> 4); o++;
					imagedata[o] = (byte)(idx & 0xf); o++;
				}
			}
		} else if( bitdepth == 2 ) {
			for( int y = 0; y < height; y++ ) {
				for( int x = 0; x < stride; x++ ) {
					int idx = image[p]&0xff; p++;
					imagedata[o] = (byte)(idx >>> 6); o++;
					imagedata[o] = (byte)((idx >>> 4) & 0x3); o++;
					imagedata[o] = (byte)((idx >>> 2) & 0x3); o++;
					imagedata[o] = (byte)(idx & 0x3); o++;
				}
			}
		} else if( bitdepth == 1 ) {
			for( int y = 0; y < height; y++ ) {
				for( int x = 0; x < stride; x++ ) {
					int idx = image[p]&0xff; p++;
					for( int b = 7; b >= 0; b-- ) {
						imagedata[o] = (byte) ((idx >>> b) & 0x1); o++;
					}
				}
			}
		} else {
			Message.throwExceptionMessage( "Not supported bit depth." );
		}
		IndexedBitmap result = new IndexedBitmap(width,height,image);
		return result;
		/*
		...
		IntBuffer bmpimage = IntBuffer.wrap(imagedata);
		bmp.copyPixelsFromBuffer(bmpimage);
		return bmp;
		*/
	}
	private static byte[] readImage(BinaryStream src, int size, int width, int height, byte bitdepth ) throws TJSException {
	...
	private static int[] readPalette( BinaryStream src, final int size ) throws TJSException {
		...
		for( int i = 0; i < len; i++ ) {
			...
			final int color = 0xff000000 | ((pal[p]&0xff)) | ((pal[p+1]&0xff) << 8) | ((pal[p+2]&0xff)<<16); // ABGR
			result[i] = color;
			p += 3;
		}
		return result;
